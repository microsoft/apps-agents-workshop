...
# Managed Operations: Inventory & Monitoring
...
| Lab | Description |
|-----|-------------|
| [Open inventory](inventory-and-monitoring-modules/01-open-inventory.md) | Open Power Platform inventory, search it, and drill from a row into resource details and connectors |
| [Add columns](inventory-and-monitoring-modules/02-add-columns.md) | Add the **Last modified by** column and sort by newest changes to track what's moving |
| [Filter inventory](inventory-and-monitoring-modules/03-filter-inventory.md) | Combine environment, owner, and date filters to narrow inventory to a precise list |
| [Export inventory to CSV](inventory-and-monitoring-modules/04-export-inventory-csv.md) | Download a filtered inventory export for offline analysis and sharing |
| [Query with Azure Resource Graph](inventory-and-monitoring-modules/05-query-resource-graph.md) | Run KQL against the `PowerPlatformResources` table to rank environments and analyze connector impact |
| [Explore the Usage tab](inventory-and-monitoring-modules/06-explore-usage-tab.md) | Review adoption trends and the most used apps, flows, and agents — tenant-wide and per environment |
| [Export inventory with a cloud flow](inventory-and-monitoring-modules/07-export-inventory-flow.md) | Build a Power Automate flow that pages through the inventory API and writes to Excel |
| [Monitor overview](inventory-and-monitoring-modules/08-monitor-overview.md) | Inspect health charts and recommendations for apps and flows |
| [View and export logs](inventory-and-monitoring-modules/09-view-logs.md) | Drill from a metric into the runtime events behind it and export them |
| [Create a custom alert rule](inventory-and-monitoring-modules/10-create-alert-rule.md) | Set up a threshold-based alert on a Managed Environment with email notifications |
| [Review triggered and predefined alerts](inventory-and-monitoring-modules/11-review-alerts.md) | See where triggered alerts land and meet the predefined alerts Microsoft runs for you |
