...
|-------|--------|-------------|
| 1 | [Customer-managed keys (CMK)](security-modules/cmk/README.md) | Encrypt the environment with a key you create, rotate, and can revoke in your own Azure Key Vault |
| 2 | [IP firewall in Dataverse](security-modules/ip-firewall/README.md) | Restrict access to the environment to allow-listed IP ranges, enforced in real time at the network layer |
| 3 | [IP cookie binding in Dataverse](security-modules/ip-cookie-binding/README.md) | Protect sessions against cookie replay attacks by binding each cookie to the IP address it was issued to |
